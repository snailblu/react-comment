...
export const getCharacterImageUrl = (
  ...
  if (!characterName || !(characterName in characterSprites)) return null;

  const characterSpriteSet = characterSprites[characterName];
  if (!characterSpriteSet) return null;

  const currentExpression = expression || 'normal';
  // expression에 해당하는 스프라이트 반환, 없으면 'normal' 반환
  return characterSpriteSet[currentExpression] || characterSpriteSet['normal'] || null;
};
