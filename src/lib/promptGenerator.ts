...
export const generateCommentPrompt = (
  ...
  // 사용자 댓글 영향력 강조 지침 생성 (수정됨: 기사 찬반 여부 명확화)
  const playerCommentInfluenceInstruction = lastPlayerComment
    ? `*   **사용자 댓글 영향력 강조:** 사용자가 마지막으로 작성한 댓글 "[ID: ${lastPlayerComment.id}] ${lastPlayerComment.nickname || '익명'}: ${lastPlayerComment.content}"이 **기사 본문의 내용에 찬성하는지 반대하는지**를 **가장 중요하게** 판단하여 예상 추천/비추천 수를 예측해주세요.
    *   **만약 사용자 댓글이 기사 내용에 명백히 찬성한다면:** 예상 **추천 수**를 현재 값(${reactions.likes})보다 **최소 50 이상 대폭 늘리고**, 예상 **비추천 수**는 현재 값(${reactions.dislikes})보다 **최소 30 이상 대폭 줄이세요**. (결과가 음수가 되면 0으로 처리)
    *   **만약 사용자 댓글이 기사 내용에 명백히 반대한다면:** 예상 **비추천 수**를 현재 값(${reactions.dislikes})보다 **최소 50 이상 대폭 늘리고**, 예상 **추천 수**는 현재 값(${reactions.likes})보다 **최소 30 이상 대폭 줄이세요**. (결과가 음수가 되면 0으로 처리)
    *   **만약 사용자 댓글의 찬반 입장이 애매하다면:** 현재 추천/비추천 수와 다른 댓글들의 분위기를 종합적으로 고려하되, 사용자 댓글의 영향력은 크게 반영하지 마세요. (중립 제거)
    *   **핵심:** 사용자 댓글의 기사에 대한 명확한 찬성 또는 반대 입장이 다른 모든 요인보다 **압도적으로 중요**하며, 예측 결과에 **결정적인 영향**을 미쳐야 합니다.`
    : '*   **사용자 댓글 없음:** 사용자가 작성한 댓글이 없으므로, 다른 댓글들의 전반적인 분위기와 현재 추천/비추천 수(${reactions.likes}/${reactions.dislikes})를 바탕으로 예상치를 생성해주세요.';
...
${playerCommentInfluenceInstruction}
    *   현재 기사의 추천/비추천 수(${reactions.likes}/${reactions.dislikes})와 댓글들의 전반적인 분위기도 참고하세요.
    *   최종 예상 추천/비추천 수를 **정확히** 다음 JSON 형식으로 프롬프트 가장 마지막 줄에 반환해주세요: \`{"likes": 예상_추천수, "dislikes": 예상_비추천수}\`
