...
const ArticleReactions: React.FC<ArticleReactionsProps> = ({ likes, dislikes, onLike, onDislike }) => {
  ...
  const handleLike = () => {
    if (liked || disliked) { // 이미 개념 또는 비추를 눌렀다면
  ...
  return (
    <div className={styles.reactionsContainer}>
      ...
      <Button variant="ghost" size="icon" onClick={handleLike} className={styles.likeButton}>
        {/* <ThumbsUp className="h-6 w-6" /> */}
        <Star className="h-6 w-6 fill-current" /> {/* 별 아이콘으로 변경 및 채우기 */}
        <span className={styles.buttonText}>개념</span>
      </Button>
      {/* onClick 핸들러 변경, disabled 속성 및 관련 className 제거 */}
      <Button variant="ghost" size="icon" onClick={handleDislike} className={styles.dislikeButton}>
