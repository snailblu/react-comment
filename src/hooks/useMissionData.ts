...
const useMissionData = (missionId: string | undefined): UseMissionDataResult => {
  const [missionData, setMissionData] = useState<Mission | null>(null);
  const [initialComments, setInitialComments] = useState<Comment[]>([]);
  const [initialOpinion, setInitialOpinion] = useState<Opinion>({ positive: 50, negative: 50 }); // 기본값 설정
  const [initialLikes, setInitialLikes] = useState(0);
  const [initialDislikes, setInitialDislikes] = useState(0);
  const [initialMonologue, setInitialMonologue] = useState('댓글을 달아 여론을 조작하자...'); // 기본값 설정
  ...
  useEffect(() => {
    ...
    const fetchMissionData = async () => {
      ...
      try {
        ...
        console.log(`useMissionData: Initializing with data for mission ${missionId}`, currentMission);
        setMissionData(currentMission);

        // 초기 상태 설정
        setInitialOpinion(currentMission.initialOpinion ?? { positive: 60, negative: 40 }); // neutral 제거 및 기본값 조정
        setTotalAttempts(currentMission.totalAttempts ?? 5);
        setInitialMonologue(currentMission.initialMonologue ?? '댓글을 달아 여론을 조작하자...');
        ...
        setError(err instanceof Error ? err.message : "미션 데이터를 불러오는 데 실패했습니다.");
