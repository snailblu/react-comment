...
import { ScriptData, GameFlags, GameStateHook } from "../types"; // SceneType 제거
  ...
  const loadGame = useCallback(() => {
    ...
    if (savedData) {
      try {
        ...
        if (
          typeof parsedData.currentScriptIndex === "number" &&
          parsedData.currentScriptIndex >= 0 &&
          (scriptData.length === 0 ||
            parsedData.currentScriptIndex < scriptData.length)
        ) {
          // scriptData가 아직 로드되지 않았거나 범위 내일 때
          setCurrentScriptIndex(parsedData.currentScriptIndex);
          console.log(
            "useGameState: 인덱스 불러오기 완료:",
            parsedData.currentScriptIndex
          );

          // gameFlags 불러오기 및 상태 업데이트
          if (
            typeof parsedData.gameFlags === "object" &&
            parsedData.gameFlags !== null
          ) {
            setGameFlags(parsedData.gameFlags);
            console.log(
              "useGameState: 플래그 불러오기 완료:",
              parsedData.gameFlags
            );
          } else {
            setGameFlags({}); // 저장된 플래그가 없거나 유효하지 않으면 초기화
            console.log(
              "useGameState: 저장된 플래그 없음 또는 유효하지 않음, 플래그 초기화"
            );
          }

          // currentEpisodeId 불러오기 및 상태 업데이트
          if (
            typeof parsedData.currentEpisodeId === "string" ||
            parsedData.currentEpisodeId === null
          ) {
            setCurrentEpisodeId(parsedData.currentEpisodeId);
            console.log(
              "useGameState: 에피소드 ID 불러오기 완료:",
              parsedData.currentEpisodeId
            );
          } else {
            setCurrentEpisodeId(null); // 유효하지 않으면 초기화
            console.log(
              "useGameState: 저장된 에피소드 ID 없음 또는 유효하지 않음, 초기화"
            );
          }
          ...
          if (typeof parsedData.sceneProgress === "string") {
            ...
            console.log(
              "useGameState: 씬 진행 상태 불러오기 완료:",
              parsedData.sceneProgress
            );
          } else {
            setSceneProgress("intro"); // 유효하지 않으면 초기화
            console.log(
              "useGameState: 저장된 씬 진행 상태 없음 또는 유효하지 않음, 초기화"
            );
          }

          // currentMissionId 불러오기 및 상태 업데이트
          if (
            typeof parsedData.currentMissionId === "string" ||
            parsedData.currentMissionId === null
          ) {
            setCurrentMissionId(parsedData.currentMissionId);
            console.log(
              "useGameState: 미션 ID 불러오기 완료:",
              parsedData.currentMissionId
            );
          } else {
            setCurrentMissionId(null);
            console.log(
              "useGameState: 저장된 미션 ID 없음 또는 유효하지 않음, 초기화"
            );
          }

          // remainingAttempts 불러오기 및 상태 업데이트
          if (
            typeof parsedData.remainingAttempts === "number" &&
            parsedData.remainingAttempts >= 0
          ) {
            setRemainingAttempts(parsedData.remainingAttempts);
            console.log(
              "useGameState: 남은 시도 횟수 불러오기 완료:",
              parsedData.remainingAttempts
            );
          } else {
            setRemainingAttempts(0); // 기본값 또는 미션 데이터 기반 초기값 설정 필요
            console.log(
              "useGameState: 저장된 남은 시도 횟수 없음 또는 유효하지 않음, 초기화"
            );
          }

          // articleLikes 불러오기 및 상태 업데이트
          if (
            typeof parsedData.articleLikes === "number" &&
            parsedData.articleLikes >= 0
          ) {
            setArticleLikes(parsedData.articleLikes);
            console.log(
              "useGameState: 기사 좋아요 수 불러오기 완료:",
              parsedData.articleLikes
            );
          } else {
            setArticleLikes(0);
            console.log(
              "useGameState: 저장된 기사 좋아요 수 없음 또는 유효하지 않음, 초기화"
            );
          }

          // articleDislikes 불러오기 및 상태 업데이트
          if (
            typeof parsedData.articleDislikes === "number" &&
            parsedData.articleDislikes >= 0
          ) {
            setArticleDislikes(parsedData.articleDislikes);
            console.log(
              "useGameState: 기사 싫어요 수 불러오기 완료:",
              parsedData.articleDislikes
            );
          } else {
            setArticleDislikes(0);
            console.log(
              "useGameState: 저장된 기사 싫어요 수 없음 또는 유효하지 않음, 초기화"
            );
          }
          ...
          console.warn(
            "useGameState: 저장된 인덱스가 유효하지 않거나 스크립트 범위를 벗어납니다:",
            parsedData.currentScriptIndex
          );
