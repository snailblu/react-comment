...
import {
  ...
  HarmCategory,
  HarmBlockThreshold,
} from "@google/generative-ai"; // Gemini SDK import
                ...
                .map((comment: Comment, index: number) => {
                  ...
                  const replyIndicator =
                    comment.isReply && comment.parentId
                      ? ` -> [${comment.parentId.substring(0, 6)}...] `
                      : ""; // 대댓글 표시 및 부모 ID 축약
                  // comment.ip가 없을 경우 기본값 제공
                  return `${index + 1}. ${prefix}${
                    comment.nickname || "익명"
                  }(${comment.ip || "?.?.?.?"})${replyIndicator}: ${
                    comment.content
                  }`;
                })
                .join("\n")
...
**주의:** 플레이어의 댓글 내용을 인용할 때는, 해당 댓글이 대댓글일 경우 앞에 붙는 대댓글 대상 ID (\`-> [ID...]\` 형식)는 반드시 제외하고 실제 댓글 내용만 언급해주세요.
...
`;
        ...
        if (!generatedText) {
          if (response.promptFeedback?.blockReason) {
            ...
            throw new Error(
              `Gemini API 요청이 안전 설정에 의해 차단되었습니다: ${response.promptFeedback.blockReason}`
            );
          } else {
            console.error(
        ...
        if (feedbackMatch) {
          ...
          // 형식 검증 (선택 사항)
          if (
            typeof feedbackData.npc_name !== "string" ||
          ...
          console.warn(
            `Failed to find "X: " pattern in Gemini response. Using cleaned text: "${cleanedText}"`
          );
