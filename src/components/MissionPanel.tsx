...
import { Target, Tags, ListChecks, Repeat, ThumbsUp, ThumbsDown, HelpCircle } from 'lucide-react'; // Import icons including ThumbsUp/Down
import {
  Card,
  CardContent,
  CardHeader,
      ...
      <CardContent className="pt-4 space-y-4 text-sm"> {/* Adjusted padding and spacing */}
        ...
        <div>
          ...
          <div className="mt-2 pl-6 space-y-1">
            {mission.goal && Object.entries(mission.goal).map(([key, value]) => {
              const { icon: Icon, text: goalText, color } = getGoalDisplay(key);
              return (
        ...
        {mission.keywords && mission.keywords.length > 0 && ( // missionData -> mission
          <div>
            <strong className="font-medium text-muted-foreground flex items-center">
              <Tags className="inline-block mr-2 h-4 w-4" /> {/* Keywords Icon */}
              키워드:
            </strong>
            <div className="mt-2 flex flex-wrap justify-center gap-2 pl-6"> {/* Indent content */}
              {mission.keywords.map((keyword: string, index: number) => ( // Add types (missionData -> mission)
                <Badge key={index} variant="secondary"> {/* Changed variant for subtle look */}
                  {keyword}
                </Badge>
              ))}
            </div>
        ...
        {mission.conditions && mission.conditions.length > 0 && ( // missionData -> mission
          <div>
            <strong className="font-medium text-muted-foreground flex items-center">
              <ListChecks className="inline-block mr-2 h-4 w-4" /> {/* Conditions Icon */}
              조건:
            </strong>
            <ul className="list-none pl-6 mt-1 space-y-1 text-foreground"> {/* Indent content */}
              {mission.conditions.map((condition: string, index: number) => ( // Add types (missionData -> mission)
                <li key={index} className="flex items-center">
                  <span className="mr-2 text-muted-foreground">-</span> {/* Custom bullet */}
                  {condition}
                </li>
              ))}
            </ul>
        ...
        {typeof attemptsLeft === 'number' && typeof totalAttempts === 'number' && totalAttempts > 0 && (
          <div>
            <strong className="font-medium text-muted-foreground flex items-center">
              <Repeat className="inline-block mr-2 h-4 w-4" /> {/* Turn Icon */}
              턴:
            </strong>
            <p className="mt-1 pl-6 text-foreground">
              {/* 현재 턴 계산: 총 턴 - 남은 턴 + 1 */}
              {totalAttempts - attemptsLeft + 1} / {totalAttempts} ({attemptsLeft} 남음) {/* 남은 턴 정보 추가 */}
            </p>
          </div>
        )}
      </CardContent>
