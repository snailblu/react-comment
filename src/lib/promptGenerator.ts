...
export const generateCommentPrompt = (
  ...
  const playerCommentInfluenceInstruction = lastPlayerComment
    ? `*   **사용자 댓글 영향력 강조:** 사용자가 마지막으로 작성한 댓글 "[ID: ${lastPlayerComment.id}] ${lastPlayerComment.nickname || '익명'}: ${lastPlayerComment.content}"의 긍정/부정 성향을 **매우 중요하게** 고려하여 예상 추천/비추천 수를 예측해주세요. 사용자 댓글이 긍정적이라면 예상 추천 수를 **대폭** 늘리고 비추천 수는 소폭 줄이거나 유지하세요. 사용자 댓글이 부정적이라면 예상 비추천 수를 **대폭** 늘리고 추천 수는 소폭 줄이거나 유지하세요. 사용자 댓글의 영향력이 다른 모든 댓글을 합친 것보다 **더 크다**고 가정하고 예측해야 합니다.`
    : '*   **사용자 댓글 없음:** 사용자가 작성한 댓글이 없으므로, 다른 댓글들의 전반적인 분위기와 현재 추천/비추천 수를 바탕으로 예상치를 생성해주세요.';
...
*   **댓글 생성 시 사용자 영향력:** 생성하는 댓글들의 내용과 분위기는 사용자의 마지막 댓글("[ID: ${lastPlayerComment?.id || '없음'}] ${lastPlayerComment?.nickname || '익명'}: ${lastPlayerComment?.content || '내용 없음'}")에 약 **60%** 정도 영향을 받아야 합니다. 사용자 댓글의 논조(긍정/부정/중립)와 유사한 방향으로 댓글을 생성하되, 완전히 동일하지는 않게 다양성을 유지해주세요. (사용자 댓글이 없다면 이 지침은 무시합니다.)
