...
export const generateAiComments = async (
  ...
  if (!missionData) {
    // 간단한 입력값 검증은 유지 가능
    return { generatedComments: [], predictedAddedReactions: null, error: "미션 데이터가 없습니다." };
  }

  try {
    console.log(`Requesting AI comments via serverless function for article: ${missionData.articleTitle}`);
    ...
    if (!response.ok) {
      // 서버에서 오류 응답을 보낸 경우
      const errorData = await response.json().catch(() => ({ error: 'Failed to parse error response from server.' })); // 오류 응답 파싱 실패 대비
      console.error(`Serverless function error: ${response.status} ${response.statusText}`, errorData);
      return {
        generatedComments: [],
        predictedAddedReactions: null,
        error: `서버 오류 (${response.status}): ${errorData?.error || response.statusText}`
      };
    }
    ...
    if (data.error) {
        console.error("Serverless function returned an error in the response body:", data.error);
        return { generatedComments: [], predictedAddedReactions: null, error: data.error };
    }
    ...
    const errorMessage = error instanceof Error ? error.message : '클라이언트 측에서 서버리스 함수 호출 중 알 수 없는 오류가 발생했습니다.';
    return { generatedComments: [], predictedAddedReactions: null, error: `네트워크 또는 클라이언트 오류: ${errorMessage}` };
  }
};
